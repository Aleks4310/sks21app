...
залогинится под демо-аккаунтом этой роли. Адрес API
(`mobile/lib/services/api_service.dart`): web → `localhost`,
устройство/эмулятор → `10.0.2.2`. Для **реального телефона** укажите IP вашего
ПК при сборке:
...
Узнать IP ПК: `ipconfig` (строка IPv4 в вашей Wi-Fi сети). Телефон должен быть
в той же сети, на ПК — открыт порт 5000 в брандмауэре.
