...
class _QuestCard extends StatelessWidget {
  ...
  void _completeQuest(BuildContext context) async {
    final provider = context.read<AppProvider>();
    final messenger = ScaffoldMessenger.of(context);
    final ok = await provider.completeQuest(quest.id);
    if (ok) {
      ...
      messenger.showSnackBar(
        SnackBar(
          content: Text(provider.lastError ?? 'Не удалось выполнить квест'),
...
class _QuestCreatorScreenState extends State<QuestCreatorScreen> {
  ...
  void _save() async {
    ...
    messenger.showSnackBar(
      SnackBar(
          content: Text(provider.lastError == null
              ? 'Квест создан и опубликован!'
              : 'Ошибка: ${provider.lastError}'),
          backgroundColor:
              provider.lastError == null ? AppTheme.success : Colors.red),
    );
  }
